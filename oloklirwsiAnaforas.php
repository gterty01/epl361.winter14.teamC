...
if(isset($_POST['submitSimeio'])){
   $_SESSION['simeioparalavis']=$_POST['simeio'];
}else{
  header("Location: index.php");
die;  	    	
...
<body>
   ...
   <div class="header-top">
	   <div class="wrap"> 
			<div class="header-top-left" style="height: 57px">
			    				    <div class="box1">
...
echo			"</ul>";
	...
	<h5 class="title" >Σημείο Παραλαβής</h5>
		<div class="clear"></div>
				<p class="m_text2"><?php echo $_SESSION['simeioparalavis']; ?></p>
				<br>
				<p class="m_text2">Παραλάβετε την παραγγελία σας επιδεικνύοντας αυτήν την αναφορά εκτυπωμένη στο σημείο που επιλέξατε.</p>
				<p class="m_text2">Η πληρωμή γίνεται με αντικαταβολή, την ημέρα παραλαβής από το σημείο που έχετε επιλέξει.</p>
...
$thisdate = date("Y-m-d");
$rec="";
$antikat=$_SESSION['simeioparalavis'];
$sql="INSERT INTO `cyfoodmuseum`.`ORDERS` (`userEmail`, `DateCreated`, `Price`, `Completed`, `Canceled`, `Weight`, `antikataboli`) VALUES ('$xristis','$thisdate','$sinolo','0', '0', '$sinolikovaros', '$antikat');";
$conn->query($sql);
$erwtima = "SELECT * FROM `ORDERS` WHERE `userEmail`='$xristis' AND `postalSent` IS NULL  AND  `citySent` IS NULL AND `Price`=$sinolo AND `countrySent` IS NULL AND `AddressSent` IS NULL AND `Completed`='0' AND `Canceled`='0' AND `Weight`='$sinolikovaros' AND `DateCreated`='$thisdate' AND `antikataboli`='$antikat' AND `receipts` IS NULL";
...
$to=$xristis;
$message = "Σας ενημερώνουμε ότι η παραγγελία σας και τα απαραίτητα στοιχεία παραλαβής έχουν καταχωρηθεί στο σύστημά μας. \r\n Αν υπάρξει κάποιο πρόβλημα με τα στοιχεία πληρωμής σας, θα σας ειδοποιήσουμε άμεσα με email. \r\n Ευχαριστούμε CyFoodMuseum.";
