...
<div class="wrap">
	<form method="post" action="manageOrdersNotCompleted_databaseJCC.php" name="formadiagrafis">
		<h4 class="title"> Παραγγελιες που εκκρεμουν</h4>'
		<h1 class="remove">Παραγγελίες με Αποστολή από το CyFoodMuseum</h1>'
  ...
  <table class="tsc_tables2_1" summary="Cart of User" style="width:75%; align:center;" id="proiontakalathiou">
    <thead>
      <tr style="vertical-align:middle">
        ...
        <th scope="col" style="height: 46px">Διεύθυνση Αποστολής</th>
        ...
        <th scope="col" style="height: 46px">Απόδειξη Πληρωμής </th>      
      </tr>
    </thead>
    <tbody>
    <?php
		...
		if($result->num_rows > 0){
			while($row = $result->fetch_assoc()) {
				if ($row['antikataboli']=="ΟΧΙ"){
					...
					echo '<tr onclick="window.location.href= \'AdminProductsOfOrderJCC.php?order='.urlencode($code).' \'">';
					echo "<td>$email  </td>";
					...
					echo "<td><a href='Receipts/Receipt$code.pdf'><img src='images/pdf.png' border='0'></a></td>"; 
					echo "</tr>";
				}
			}
		}
		...
		if($result->num_rows > 0){
			while($row = $result->fetch_assoc()) {
				if ($row['antikataboli']!="ΟΧΙ"){
					...
					echo '<tr onclick="window.location.href= \'AdminOrderAntikatavoliJCC.php?order='.urlencode($code).' \'">';
					echo "<td>$email  </td>";
					echo "<td>$price   </td>";
					echo "<td>$weight  </td>";
					echo "<td>$date </td>";
