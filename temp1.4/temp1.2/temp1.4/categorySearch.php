...
if (!$conn->set_charset("utf8")) {
    ...
    //printf("Current character set: %s\n", $conn->character_set_name());
		...
		$querys ="SELECT DISTINCT `Code` , `Name` , `Description` , `Price` , `NameCat` , `Weight` , `Availability` , `image` ,  `CompanyName` FROM `PRODUCT`, `SUPPLIER` , `CATEGORY` where `CodeOfCategory`='$katigoria' AND `CodeOfCategory` = `CodeCat` AND `CodeOfSupplier` = `SupplierNumber` AND `Availability`>0";
...
if($result->num_rows > 0){
	...
	//echo  "<iframe name='votar' style='display:none;' sandbox='allow-iframes allow-same-origin allow-scripts allow-modals' ></iframe>";
  	...
  	echo   "<form id='prosthiki' name='prosthiki' method='POST' action='prosthikiCat.php'  accept-charset='UTF-8'>"; 
